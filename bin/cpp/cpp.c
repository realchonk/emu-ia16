...
# define cputc(a,b)	if(walkifstack ()) putc(a,b)
...
#else /* !pdp11 */
# define SBSIZE 30000		/* std = 12000, wnj aug 1979 */
char	sbf[SBSIZE];
#endif /* pdp11 */
char	*savch	= sbf;
...
STATIC	char	ifstack[MAX_IFSTACK];
STATIC	int	iflen;
...
pushif (x)
{
	if (iflen >= MAX_IFSTACK) 
		pperror ("too many nested #if's");

	ifstack[iflen++] = x ? (IF_VAL | IF_HAS) : 0;
	return 0;
}
...
register char *p;
{
	...
	for (;;) {
		if (mactop > inctop[ifno]) { /* retrieve hunk of pushed-back macro text */
			...
			maclvl = 0;
			if ((ninbuf = read (fin, pbuf, BUFSIZ)) > 0) {
				pend = pbuf + ninbuf;
				*pend = '\0';
				return p;
			}
			...
			if (ifno == 0) { /* end of input */
				if (plvl != 0) {
					...
					pperror ("%s: unterminated macro call", (size_t)macnam, 0);
					lineno[ifno] = tlin;
					fnames[ifno] = tfil;
					np = p;
...
register char *p;
{
	...
	if (mactop >= MAXFRE) {
		pperror ("%s: too much pushback", (size_t)macnam, 0);
		p = inp = pend;
...
register char *p;
{
	...
	if (*inp++ == '<') { /* special <> syntax */
		inctype = 1;
		pushif (0);	/* prevent macro expansion */
		...
		popif ();	/* reenable macro expansion */
		...
		pperror ("bad include syntax", 0, 0);
		inctype = 2;
	}
	...
	if (ifno + 1 >= MAXINC) {
		pperror ("Unreasonable include nesting", 0, 0);
		return p;
	}
	...
	for (dirp = dirs + inctype; *dirp; ++dirp) {
		...
		if ((fins[ifno+1] = open (nfil, READ)) > 0) {
			filok = 1;
			fin = fins[++ifno];
			break;
		}
	...
	if (filok == 0) {
		pperror ("Can't find include file %s", (size_t)filname, 0);
	} else {
		lineno[ifno] = 1;
		fnames[ifno] = cp = nfil;
		while (*cp++);
...
{ /* process '#define' */
	...
	if (savch > sbf + SBSIZE - BUFSIZ) {
		pperror ("too much defining", 0, 0);
		return p;
	}

	oldsavch = savch; /* to reclaim space if redefinition */
	pushif (0); /* prevent macro expansion during 'define' */
	p = skipbl(p);
	pin = inp;
	...
	if (*pin == '(') { /* with parameters; identify the formals */
		...
		for (;;) {
			...
			if (*pin == '\n') {
				--lineno[ifno];
				--p;
				pperror ("%s: missing )", (size_t)np->name, 0);
				break;
			}
			...
			if ((toktyp + COFF)[*pin] != IDENT) {
				c = *p;
				*p = '\0';
				pperror ("bad formal: %s", (size_t)pin, 0);
				*p = c;
			} else if (pf >= &formal[MAXFRM]) {
				c = *p;
				*p = '\0';
				pperror ("too many formals: %s", (size_t)pin, 0);
				*p = c;
			} else {
				*pf++ = cf;
				while (pin < p)
	...
	*psav++ = '\0';
	...
	if ((cf = oldval) != NULL) { /* redefinition */
		...
		if (strcmp (++cf, oldsavch) != 0) {/* redefinition different from old */
			--lineno[ifno];
	...
	for (;;) {
		...
		*--inp = SALT;
		...
		if (np == defloc) { /* define */
			...
			setif (!(ifstack[iflen - 1] & IF_HAS));
			if (walkifstack ())
				sayline (CONT);
		} else if (np == elifloc) { /* elif */
			newp = p;
			setif (!(ifstack[iflen - 1] & IF_HAS) && (yyparse () != 0));
			p = newp;
...
struct symtab *sp;
{
	...
	if ((p - macforw) <= macdam) {
		if (++maclvl > symsiz && !rflag) {
			pperror ("%s: macro recursion", (size_t)sp->name, 0);
			return p;
		}
	} else {
	...
	if ((params = *--vp & 0xFF) != 0) {/* definition calls for params */
		register char **pa;
		ca = acttxt;
		pa = actual;
		...
		sloscan ();
		pushif (0); /* no expansion during search for actuals */
		plvl = -1;
			...
			for (plvl = 1; plvl != 0; ) {
				*ca++ = '\0';
				...
				for (;;) {
					...
					if (ca > &acttxt[BUFSIZ])
						pperror ("%s: actuals too long", (size_t)sp->name, 0);
				}

				if (pa >= &actual[MAXFRM]) {
					ppwarn (match, (size_t)sp->name);
				} else {
					actused[pa - actual] = 0;
					*pa++ = ca;
				}
...
char *argv[];
{
		...
		case '-':
			...
			case 'D':
				  if (predef > prespc + NPREDEF) {
					  pperror ("too many -D options, ignoring %s", (size_t)argv[i], 0);
					  continue;
				  }
			...
			case 'U':
				  if (prund > punspc + NPREDEF) {
					  pperror ("too many -U options, ignoring %s", (size_t)argv[i], 0);
					  continue;
				  }
				  *prund++ = argv[i]+2;
				  continue;
			case 'I':
				  if (nd > 8) {
					  pperror ("excessive -I file (%s) ignored", (size_t)argv[i], 0);
				  } else {
					  dirs[nd++] = argv[i] + 2;
				  }
				  continue;
			case '\0':
				  continue;
			default: 
				   pperror ("unknown flag %s", (size_t)argv[i], 0);
				   continue;
			}
		default:
			if (fin == STDIN) {
				if ((fin = open (argv[i], READ)) < 0) {
					pperror ("No source file %s", (size_t)argv[i], 0);
					exit (8);
				}
				...
				if ((fout = fopen (argv[i], "w")) == NULL) {
					pperror ("Can't create %s", (size_t)argv[i], 0);
					exit(8);
				} else {
					fclose (stdout);
				}
			} else {
				pperror ("extraneous name %s", (size_t)argv[i], 0);
			}
		}
	}
	...
	if (mflag) {
		...
		mout = fout;
		setbuf (mout, NULL);
		if ((fout = fopen ("/dev/null", "w")) == NULL) {
			pperror ("Can't open /dev/null", 0, 0);
			exit (8);
		}
	}
	setbuf (fout, obuf);
