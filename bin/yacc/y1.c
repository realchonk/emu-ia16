...
struct item
    *pstate[NSTATES + 2]; /* pointers to the descriptions of the states */
...
struct looksets *pfirst[NNONTERM + 2]; /* vector of pointers to first sets for
                                          each nonterminal */
...
char *
symnam(i) { /* return a pointer to the name of symbol i */
	register char *cp;
...
summary() { /* output the summary on the tty */
	...
	if (foutput != NULL) {
		...
		fprintf(
		    ...
		    zzsrconf, zzrrconf);
		fprintf(foutput, "%zd/%d working sets used\n", zzcwp - wsets,
		        WSETSIZE);
		fprintf(foutput, "memory: states,etc. %zd/%d, parser %zd/%d\n",
...
cpres() { /* compute an array with the beginnings of  productions yielding given
...
state(c) { /* sorts last state,and sees if it equals earlier ones. returns state
	      number */
	int size1, size2;
	register i;
	...
	/* state is new */
	if (nolook)
		error("yacc state/nolook error", 0);
	pstate[nstate + 2] = p2;
	if (nstate + 1 >= NSTATES)
...
again:
	PLOOP(1, i) {
		if (pempty[*prdptr[i] - NTBASE] ==
		    WHOKNOWS) { /* not known to be empty */
			for (p = prdptr[i] + 1;
...
more:
	SLOOP(i) {
		...
		WSLOOP(wsets, p) { /* generate goto's */
			...
			/* do a goto on c */
			WSLOOP(p, q) {
				if (c == *(q->pitem)) { /* this item contributes
					                   to the goto */
...
struct looksets *p;
{
	...
	q = &lkst[nlset++];
	if (nlset >= LSETSIZE)
		error("too many lookahead sets", 0);
	SETLOOP(j) {
		q->lset[j] = p->lset[j];
	}
	return q;
