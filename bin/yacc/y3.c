...
go2gen(c) { /* output the gotos for nonterminal c */
	...
	if (g2debug && foutput != NULL) {
		...
		NTLOOP(i)
		if (temp1[i]) fprintf(foutput, "%s ", nontrst[i].name);
		fprintf(foutput, "\n");
	}
...
/* r is a rule number, t a token number */
	...
	if (PLEVEL(lt) == 0 || PLEVEL(lp) == 0) {
		...
		if (foutput != NULL)
			fprintf(foutput,
			        "\n%d: shift/reduce conflict (shift %d, red'n "
			        "%d) on %s",
