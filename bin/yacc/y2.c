...
char *argv[];
{
	...
	/* added production */
	*mem++ = NTBASE;
	*mem++ = start; /* if start is 0, we will overwrite with the lhs of the
	                   first rule */
	*mem++ = 1;
	*mem++ = 0;
	prdptr[1] = mem;
...
cpyunion() {
	/* copy the union declaration to the output, and the define file if
	 * present */
